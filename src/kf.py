...
class KalmanFilterCalculator(nn.Module):
    ...
    def _filtering(self,_state,_P,obs):
...
class KalamanFilter:
    ...
    def filtering(self,s_init,P_init,dts,observations,controls=None,init_obs=True):
        ...
        if init_obs:
            obs = observations[0].view(-1,1)
            s,p = self.calculator._filtering(s.view(-1,1),p,obs)
            Ss_f.append(s.data.clone())
            Ps_f.append(p.data.clone())
        else:
            Ss_f.append(s_init.view(-1,1))
            Ps_f.append(P_init)
